...
the agent can use as context when writing SQL.
"""
...
def get_table_schema(table_name: str) -> dict:
    """Return column names, types, and row count for one mart table."""
...
def get_sample_rows(table_name: str, n: int = 3) -> list[dict]:
    ...
    try:
        full_name = f"{MARTS_SCHEMA}.{table_name}"
        result    = con.execute(f"SELECT * FROM {full_name} LIMIT {n}").fetchdf()
